...
while True:
    ...
    if frame_counter % 10 == 0:
        try:
            # Realiza el análisis facial para obtener edad, género y emoción.
    ...
    if previous_results:
        for face_data in previous_results:
            ...
            # Extrae el género, la edad y la emoción
            gender_data = face_data['gender']
            gender = max(gender_data, key=gender_data.get)
            age = face_data['age']
            emotion_data = face_data['emotion']
            dominant_emotion = max(emotion_data, key=emotion_data.get)
            percentage = int(face_data['emotion'][dominant_emotion])
            
            # Traduce el género y la emoción para una mejor visualización
            ...
            dominant_emotion_es = emotion_translations.get(dominant_emotion, dominant_emotion)

            # Dibuja el rectángulo alrededor de la cara
            # El color es (0, 255, 0) que es verde, y el grosor es 2 píxeles.
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)

            # Prepara el texto para mostrar
            text = f"{gender_es}, {age} anios, {dominant_emotion_es} ({percentage}%)"
            ...
            # Escribe el texto (género, edad y emoción) justo encima del rectángulo
